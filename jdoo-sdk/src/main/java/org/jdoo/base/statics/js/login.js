...
$(function () {
    $.validator.setDefaults({
        submitHandler: function () {
            ...
            $.jrpc({
                url: $.jutil.getTenantPath() + "/rpc/login",
                data: {
                    id: $.jutil.newId(),
                    jsonrpc: "2.0",
                    method: "login",
                ...
                success: function (rs) {
                    ...
                    if (rs.error) {
                        ...
                        if (remember) {
                            $.jutil.cookie('jtoken1', rs.result.token, { expires: 7 });
                        } else {
                            $.jutil.cookie('jtoken', rs.result.token);
                        }
                        var url = $.jutil.getUrlParam('url') || $.jutil.getTenantPath();
