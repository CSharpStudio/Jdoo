package org.jdoo.core;

/**
 * 常量
 * 
 * @author lrz
 */
public class Constants {
    public final static String SUPERUSER_ID = "superuser";
    public final static String ANONYMOUS = "☒ANONYMOUS";

    public final static String NAME = "name";
    public final static String CREATE_UID = "create_uid";
    public final static String CREATE_DATE = "create_date";
    public final static String UPDATE_UID = "update_uid";
    public final static String UPDATE_DATE = "update_date";
    public final static String ID = "id";
    public final static String USER = "rbac.user";
    public final static String INIT = "init";

    public final static String PREFETCH_FIELDS = "prefetch_fields";

    public final static String ARGS = "args";
    public final static String COMPUTE = "compute";
    public final static String STORE = "store";
    public final static String STATE = "state";
    public final static String COPY = "copy";
    public final static String RELATED = "related";
    public final static String AUTOMATIC = "automatic";
    public final static String MODEL_NAME = "modelName";
    public final static String MANUAL = "manual";
    public final static String INDEX = "index";
    public final static String REQUIRED = "required";
    public final static String READONLY = "readonly";
    public final static String HELP = "help";
    public final static String LABEL = "label";
    public final static String DEPENDS = "depends";
    public final static String CRITERIA = "criteria";

    public final static String COMMA = ",";

    public final static String BOOLEAN = "boolean";
    public final static String BINARY = "binary";
    public final static String INTEGER = "integer";
    public final static String FLOAT = "float";
    public final static String IMAGE = "image";
    public final static String CHAR = "char";
    public final static String DATE = "date";
    public final static String DATETIME = "datetime";
    public final static String HTML = "html";
    public final static String SELECTION = "selection";
    public final static String TEXT = "text";
    public final static String MANY2ONE = "many2one";
    public final static String MANY2MANY = "many2many";
    public final static String ONE2MANY = "one2many";

    
    public final static String BASE = "base";
    public final static String TABLE = "table";
    public final static String LOG_ACCESS = "logAccess";
    public final static String DESCRIPTION = "description";
    public final static String ORDER = "order";
    public final static String PRESENT = "present";
    public final static String PRESENT_FORMAT = "presentFormat";
    public final static String METHOD_KEY = "methods";
    public final static String SERVICE_KEY = "services";
    public final static String UNIQUE_KEY = "uniques";

    
    public final static String BASE_PACKAGE = "org.jdoo.base";
}
